...
```{r}
# the columns are clean, will use tupper() on the values 
for (col in bool_cols) {
...
df2$activity <- as.logical(rowSums(df2[bool_cols]))
str(df2)
```
...
This one is pretty funny, I actually spent a good 30 minutes trying to figure out if my data was wrong, like FALSE was somehow being aggregated, but it just turns out all the squirrels were very active!
...
Coming from Python I like f-strings, so I made some using R. 

```{r}
paste(
  "Squirrel chasing stats:",
  colSums(df2["chasing"]),
  " / ",
